	...
	BMo_scrollGallery.prototype = {
		...
		showGallery:function(){
			...
			if(this.gallery.options.sG_responsive){
				if(this.isVertical){
					this.resizeGallery(this.$gallery_obj.width()-this.$bmo_the_gallery_thumb_area.outerWidth(true)-1,this.gallery.options.gallery_height);//zur sicherheit -1px für FF
				}else{
					this.resizeGallery(this.$gallery_obj.width(),this.gallery.options.gallery_height);
				}
			}else{
				this.resizeGallery(this.gallery.options.gallery_width,this.gallery.options.gallery_height);
			}
		...
		resizeGallery:function(gallery_width, gallery_height){//new width and height
			...
			if(this.gallery.options.sG_images){
				...
				this.$bmo_the_gallery_images.find('div.bmo_the_gallery_image').each(function(){
					...
					if($.browser.mozilla) $(this).css('line-height',(gallery_height-40)+'px'); //FF bug, todo beobachten und evtl. ändern.
					image_width_withBMP = $(this).outerWidth(true); //mit padding border und margin
				});
				this.$bmo_the_gallery_images.css('width',(image_width_withBMP * this.gallery.numberOfImgs)+'px').css('height',gallery_height+'px');
			}
		}, 
