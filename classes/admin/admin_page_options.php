...
class bmoExpoAdmin_options_page {
		...
		public function BMo_Expo_registerPageComponents ($withoutDesign = false){
			//register all page Components:

			//meta boxes - more infos http://www.wproots.com/ultimate-guide-to-meta-boxes-in-wordpress/
			add_meta_box('BMo_Exp_main_box', 'Default Gallery Options', array($this, 'BMo_Expo_main_MetaBox'), BMO_EXPO_PLUGINNAME, 'normal', 'core');
			...
			foreach($options as $key=>$option){
				...
				if($isSpecial){//special fields e.g. design
					if(!$withoutDesign){
						add_settings_field('BMo_Expo_options_field_'.$key, $option['desc'], array($this,'BMo_Expo_options_field_html_design'), 'BMo_Expo_options_section_'.$option["type"].'_el', 'BMo_Expo_options_section_'.$option["type"], array("key"=>$key,"option"=>$option));
					}
				}else{
					//else - generated fields
					switch ($option['valtype']){
						case 'bool':
							add_settings_field('BMo_Expo_options_field_'.$key, $option['desc'], array($this,'BMo_Expo_options_field_html_bool'), 'BMo_Expo_options_section_'.$option["type"].'_el', 'BMo_Expo_options_section_'.$option["type"], array("key"=>$key,"option"=>$option));
						break;
						case 'int':
							add_settings_field('BMo_Expo_options_field_'.$key, $option['desc'], array($this,'BMo_Expo_options_field_html_int'), 'BMo_Expo_options_section_'.$option["type"].'_el', 'BMo_Expo_options_section_'.$option["type"], array("key"=>$key,"option"=>$option));
						break;
						case 'string':
							add_settings_field('BMo_Expo_options_field_'.$key, $option['desc'], array($this,'BMo_Expo_options_field_html_string'), 'BMo_Expo_options_section_'.$option["type"].'_el', 'BMo_Expo_options_section_'.$option["type"], array("key"=>$key,"option"=>$option));
						break;
						case 'select':
							add_settings_field('BMo_Expo_options_field_'.$key, $option['desc'], array($this,'BMo_Expo_options_field_html_select'), 'BMo_Expo_options_section_'.$option["type"].'_el', 'BMo_Expo_options_section_'.$option["type"], array("key"=>$key,"option"=>$option));
						break;
						default:
		...
		public function BMo_Expo_options_section_common_php_html() {
		   echo '<p>Here you can change the common PHP BMo Expo options:</p>';
		 }
		 public function BMo_Expo_options_section_common_html() {
		   echo '<p>Here you can change the common BMo Expo options:</p>';
		 }
		public function BMo_Expo_options_section_sG_html() {
		   echo '<p>Scroll Gallery options:</p>';
		 }
		public function BMo_Expo_options_section_slG_html() {
		   echo '<p>Scroll Lightbox options:</p>';
		 }
		//default fields
		 ...
		 public function BMo_Expo_main_MetaBox(){
			 ...
			 echo "
			    <ul class='nav nav-tabs' id='bmoTab' data-tabs='tabs'>
			    	<li class='active'><a id='tab_common' href='#common' data-toggle='tab'>General</a></li>
			    	<li class=''><a id='tab_common_js' href='#common_js' data-toggle='tab'>General Gallery</a></li>
