...
class bmoExpoAdmin {
	...
	public function BMo_Expo_admin_menu() { 
	    ...
	    add_submenu_page(  BMO_EXPO_PLUGINNAME."-admin", 'BMo Expo Admin', 'Features & Usage', 'manage_options', BMO_EXPO_PLUGINNAME."-admin", array($this->obj_bmoExpoAdmin_general_page, 'BMo_Expo_Admin_show_page')); // add Admin Menu Usage Page
		
		$this->obj_bmoExpoAdmin_options_page = new bmoExpoAdmin_options_page($this);
		add_submenu_page(  BMO_EXPO_PLUGINNAME."-admin", 'BMo Expo Options', 'Options', 'manage_options', BMO_EXPO_PLUGINNAME."-admin-options", array($this->obj_bmoExpoAdmin_options_page, 'BMo_Expo_Admin_show_page')); // add Admin Menu Usage Page
	}
	...
	public function BMo_Expo_admin_init() {
		...
		if ($pagenow=="admin.php" && ($_GET['page']== BMO_EXPO_PLUGINNAME."-admin"||$_GET['page']== BMO_EXPO_PLUGINNAME."-admin-options")||$pagenow =="options.php") {//check if is my plugin page
			wp_register_style( 'bmo_admin_css', BMO_EXPO_URL.'/css/admin/bmo_bootstrap.min.css', array(), BMO_EXPO_VERSION ,'all');
			wp_register_style( 'bmo_admin_style_css', BMO_EXPO_URL.'/css/admin/bmo_admin_style.css', array(), BMO_EXPO_VERSION ,'all');
			wp_register_script( 'bmo_admin_js', BMO_EXPO_URL.'/js/admin/bmo_bootstrap.min.js', array(), BMO_EXPO_VERSION ,'all');
			if (function_exists('wp_enqueue_script')) {
		...
		if ($pagenow=="admin.php" && $_GET['page']== BMO_EXPO_PLUGINNAME."-admin"||$pagenow =="options.php") {//check 
			$this->obj_bmoExpoAdmin_general_page->BMo_Expo_registerPageComponents(); //spezielle Komponenten
		}
		if ($pagenow=="admin.php" && $_GET['page']== BMO_EXPO_PLUGINNAME."-admin-options"||$pagenow =="options.php") {//check 
	 ...
	 public function admin_notices() {
		...
		if ( $this->hasNGG==false && $pagenow=="admin.php" && $_GET['page']== BMO_EXPO_PLUGINNAME."-admin") {//check if NextGEN is activated
			$this->show_message("We recommend to use the plugin together with the <strong><a href='http://wordpress.org/extend/plugins/nextgen-gallery/' target='_blank'>NextGEN Gallery</a></strong> plugin.");
		}
		
		$options = get_option(BMO_EXPO_OPTIONS);
		if(empty($options)){
			$this->show_message("Error, please reinstall the <strong>BMo Expo - a Wordpress and NextGEN Gallery Exhibition Plugin</strong>!");
		}
	 }
	 ...
	 public function BMo_Expo_registerPageComponents(){
		...
		//side
		add_meta_box('BMo_Exp_meta_box', 'Do you like this Plugin?', array($this, 'BMo_Expo_like_MetaBox'), BMO_EXPO_PLUGINNAME, 'side', 'core');
	 ...
	 public function BMo_Expo_RegisterPluginLinks($links, $file) { 
		if ($file == BMO_EXPO_BASE_FILE) {
			return array_merge(
				$links,
				array( sprintf( '<a href="admin.php?page=%s">%s</a>', BMO_EXPO_PLUGINNAME."-admin-options", __('Settings') ), '<g:plusone size="small"  href="http://software.bmo-design.de/bmoExpo/wordpress-plugin-bmo-exhibition.html"></g:plusone>' )
			);
		}
		return $links;
	 } 
